/**
 * The {@code Prac7} class implements two substring search algorithms; Brute
 * force search and KMPS, both are tested and timed in main.
 *
 * @author Ahmed Jouda
 */
public class Prac7 {
	...
	public static void bruteForceSearch(String text, String pattern) {
		...
		// get length of text and pattern
		int n = text.length();
		int m = pattern.length();

		// i is a pointer in the text and j is a pointer in the pattern
		// if the outter loop finishes then there is no match
		// each i increment, j is incremented until a mismatch or pattern found
		for (int i = 0; i <= n - m; i++) {
			int j; // declared outside loop because used in if statement too
			for (j = 0; j < m; j++) {
				if (text.charAt(i + j) != pattern.charAt(j)) {
					break;
				}
			}
			// print position where the match was found in text -
			// stops at 1st occurrence of pattern
			if (j == m) {
